...
@Service
@Scope("prototype")
public class AddSaleVolumeToGoodsHandler extends SuperHandler<GoodsEntity> {
    @Autowired
    ...
    public void process(GoodsEntity goodsEntity) throws Exception {
        ...
        if(goodsEntity.getQty() <= 0) {

            throw new ClassValidationException("qty 必须大于0");
        }
        goodsService.addSaleVolumeToGoods(goodsEntity.getGoodsId(),goodsEntity.getQty());
    }
