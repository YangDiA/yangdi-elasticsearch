...
@Service
@Scope("prototype")
public class HelpCenterUpdateOneHandler extends AbstractKafkaConsumer {
    @Autowired
    ...
    public void handleMessage(String msg) throws Exception {
        try {
            ...
            HelpCenterEntity resultHelpsEntity = helpCenterService.findOne(helpCenterEntity);
                this.updateProcess(helpCenterEntity);
                this.removeProcess(resultHelpsEntity);
        } catch (Exception e) {
            throw e;
        }
    }

    private void updateProcess(HelpCenterEntity helpCenterEntity) throws Exception {
        //validating for checking property that should not be empty
        ClassValidation.validate(helpCenterEntity);
        helpCenterService.updateOne(helpCenterEntity);
    }

    private void removeProcess(HelpCenterEntity helpCenterEntity) throws Exception {
        //validating for checking property that should not be empty
        ClassValidation.validate(helpCenterEntity);
        helpCenterService.removeOne(helpCenterEntity);
    }
}